...
class VeiculoSelecionadoPage {
  ...
  checkNovosVeiculosPageElements(formatModelo) {
    ...
    cy.intercept(
      "POST",
      `https://bam.nr-data.net/1/**&ref=https://autoforce-academy.pilotodetestes.com.br/ddm-fiat-v2-dois/novos/${formatModelo}&**`,
    ).as(`postBamNovos${formatModelo}`);

    cy.url().should("include", `/${formatModelo}`);
    cy.title(this.labels.titlePage);
    headerPage.checkHeaderPageElements();

    cy.get(this.elements.breadcrumbModelo)
      .contains(this.labels.textBreadcrumbModelo)
      .should("be.visible");
    cy.get(this.elements.btnEuQuero).scrollIntoView().should("be.visible");

    // Esperas das rotas
    cy.wait(`@postBamNovos${formatModelo}`)
  ...
  createLead() {
    ...
    cy.get(this.elements.toastMensagem)
      .contains(this.labels.textSuccessMessage)
      .should("be.visible");
  }

  hasMissingFields() {
    cy.get(this.elements.divErrorFiels)
      .contains(this.labels.textErrorFields)
      .should("be.visible");
  }
  ...
  fillLead(dataObj, isValid) {
    ...
    cy.get(this.elements.titleForm)
      .contains(this.labels.titleLead)
      .should("be.visible");
    cy.get(this.elements.paragraphForm)
      .contains(this.labels.textParagraphForm)
      .should("be.visible");
    ...
    cy.get(this.elements.inputPhone)
      .should("be.visible")
      .type(dataObj.telephone);
    cy.get(this.elements.inputCPF).should("be.visible").type(dataObj.cpf);

    if (isValid) this.createLead();
    else this.hasMissingFields();
  }
}
